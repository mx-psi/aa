...
def display_figure():
    ...
    ax.plot(min_point_[0], min_point_[1], E(
        [min_point_[0], min_point_[1]]), 'r*', markersize=5)
...
resEtaPeqNewton = np.apply_along_axis(f, 1, newton(
    np.array([0.1, 0.1]), f, gradf, hessianf, 0.01, 50))
resEtaGrandeNewton = np.apply_along_axis(f, 1, newton(
    np.array([0.1, 0.1]), f, gradf, hessianf, 0.1, 50))
...
for label, initial in initial_points:
    ...
    print("{:>10}".format(
        label), points[-1, :], "{: 1.5f}".format(f(points[-1, :])), gradf(points[-1, :]))
